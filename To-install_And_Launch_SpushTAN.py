...
class Multi(): 
    ...
    def collect_deviceid(self):
        line2 = []
        pipe=subprocess.Popen(['adb', 'devices'], stdout=subprocess.PIPE, stderr=None)
        result = pipe.communicate()[0].decode('utf-8') #byte to string coversion i used decode method.
        result =result.strip()
        file = open('devicename.txt', 'w', newline='')
        file.writelines(result)
        file.close()
        with open('devicename.txt', 'r') as f:
            lines = f.readlines()[1:]
            for line in lines:
                line = line.strip()
                line1 = line.replace("\tdevice", "")
                line2.append(line1)
        f.close()        
        return line2
    ...
    def menifest(self):
        ...
        #pipe = subprocess.Popen(['aapt', 'dump', 'badging', apk2], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
        pipe = subprocess.Popen(['aapt', 'dump', 'badging', apk2], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding = "utf-8")
        ...
        #file = codecs.open('newdump.txt', 'w', encoding='utf-16-le', errors='ignore')
    ...
    def app_installation(self, apk2, packagename, activityname, line1):
        ...
        #pipe = subprocess.check_output(['adb', '-s', line2, 'install', '-r', apk2], shell=True, universal_newlines=True)
        pit = subprocess.Popen(['adb', '-s', line2, 'install', '-t', apk2], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, shell=True)
